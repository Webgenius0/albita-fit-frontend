...
const CommonButton = ({
  ...
  const classList = `block py-2.5 ${textPink ? `text-primary-pink hover:text-white` : `text-white`} ${textBold ? `font-bold` : `font-normal`} ${variant === "normal" ? `bg-primary-pink px-7` : `border-primary-pink hover:bg-primary-pink border-[2px] border-solid px-6`} h-fit w-fit rounded-[50px] ${hasArrow ? `flex items-center gap-3` : ``} duration-300 ease-in-out cursor-pointer`;
